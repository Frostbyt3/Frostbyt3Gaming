...
if ($allocationId <= 0) {
    networkJsonResponse(400, [
        'ok' => false,
        'error' => 'Missing or invalid allocation ID.',
    ]);
}

try {
    pteroEnsureServerAccessSession(false);
    pteroRequireServerPermission($serverIdentifier, 'allocation.delete');

    session_write_close();

    $result = pteroDeleteServerNetworkAllocation($serverIdentifier, $allocationId);

    if (empty($result['ok']) && (int)($result['status'] ?? 0) !== 204) {
        networkJsonResponse((int)($result['status'] ?? 500), [
            'ok' => false,
            'error' => $result['error'] ?? 'Failed to delete allocation.',
        ]);
    }
    ...
    networkJsonResponse(500, [
        'ok' => false,
        'error' => $message !== '' ? $message : 'Failed to delete allocation.',
    ]);
}