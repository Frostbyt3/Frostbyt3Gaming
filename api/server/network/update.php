...
try {
    ...
    $result = pteroUpdateServerNetworkAllocation($serverIdentifier, $allocationId, [
        'notes' => $notes,
    ]);
    ...
    if (empty($result['ok'])) {
        networkJsonResponse((int)($result['status'] ?? 500), [
            'ok' => false,
            'error' => $result['error'] ?? 'Failed to update allocation.',
        ]);
    }

    networkJsonResponse(200, [
        'ok' => true,
        'message' => 'Allocation notes updated successfully.',
        'data' => [
    ...
    networkJsonResponse(500, [
        'ok' => false,
        'error' => $message !== '' ? $message : 'Failed to update allocation.',
    ]);
}