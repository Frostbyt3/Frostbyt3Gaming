...
try {
    ...
    $result = pteroListDatabases($serverIdentifier, false);

    if (empty($result['ok'])) {
        http_response_code((int)($result['status'] ?? 500) ?: 500);
        echo json_encode([
            'ok' => false,
            'error' => $result['error'] ?? 'Failed to load databases.',
            'items' => [],
        ]);
        exit;
    }
    ...
    pteroJsonError(500, $message !== '' ? $message : 'Failed to load databases.');
}
