...
try {
    ...
    pteroRequireServerPermission($serverIdentifier, 'database.update');

    session_write_close();

    $result = pteroRotateDatabasePassword($serverIdentifier, $databaseId);

    if (empty($result['ok'])) {
        http_response_code((int)($result['status'] ?? 500) ?: 500);
        echo json_encode([
            'ok' => false,
            'error' => $result['error'] ?? 'Failed to rotate database password.',
            'data' => null,
        ]);
        exit;
    }
    ...
    pteroJsonError(500, $message !== '' ? $message : 'Failed to rotate database password.');
}
