...
try {
    ...
    $result = pteroExecuteSchedule($serverIdentifier, $scheduleId);

    if (empty($result['ok'])) {
        http_response_code((int)($result['status'] ?? 500) ?: 500);
        echo json_encode([
            'ok' => false,
            'error' => $result['error'] ?? 'Failed to execute schedule.',
            'data' => null,
        ]);
        exit;
    }
    ...
    pteroJsonError(500, $message !== '' ? $message : 'Failed to execute schedule.');
}